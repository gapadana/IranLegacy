...
function drawLevel(r, g, b, a)
	...
	if IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
		AddTextComponentSubstringPlayerName('~m~Police mode - Voice: ~s~' .. voicepolice.level)
	elseif IsPedInAnyVehicle(GetPlayerPed(-1), false) then
		AddTextComponentSubstringPlayerName('~m~Car mode - Voice: ~s~' .. voicecar.level)
	else
		AddTextComponentSubstringPlayerName('~m~Voice: ~s~' .. voice.level)
	end
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1)
			if IsControlJustPressed(1, Keys['M']) and IsControlPressed(1, Keys['LEFTSHIFT']) then
				if IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) and GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)), -1) == GetPlayerPed(-1) then
